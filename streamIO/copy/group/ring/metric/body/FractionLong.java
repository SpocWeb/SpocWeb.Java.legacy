...
//		Numerator   = 0; //not necessary
	...
	public IMetricIRing addSubAt(Object arg, boolean add) {	//watch out for Integer Arguments:
		...
		if (arg instanceof FractionLong) {
			...
			if (add) Numerator += H1;	//Flag f�r Addition/Subtraktion
			else	 Numerator -= H1;
			Denominator *= arg_.Denominator;
		} else {	//Integer Argument
			if (add) Numerator += Denominator*ByRefLong.TO_LONG(arg);	//Flag f�r Addition/Subtraktion
...
/*	public FractionLong MakeRational (double x) { //f�r schnellere Integer-Berechnungen
		IF Math.abs(x) > 1 {
