...
/**
  ...
  *
  ...
  zwischen gew�nschtem und aktuellem Ergebnis.
  Als Input/Output Paare k�nnen neben den aktuellen (und hoffentlich repr�sentativen)
  Ergebnissen auch mit Hilfe einer Regel generierte Paare dienen.
  Dadurch verinnerlicht das System die Regel und besitzt eine gr��ere Datenbasis.
  Die Lerngewichtung der k�nstlichen F�lle
  Die Menge der Datens�tze sollte die Anzahl der Neuronen wesentlich �berschreiten,
  ...
  Fakten �ber Industrie Roboter:
  -nur einsetzbar in gleichf�rmig strukturierten Umgebungen,
   daher bisher nichts weiter als reprogrammierbare Positioniermaschinen,
   auch aufgrund ihrer Steifigkeit
  -Die Gew�hnung an unregelm��ige Umgebungen ist dagegen immer wieder gescheitert,
   nicht zuletzt aus Kostengr�nden.
  -Sie k�nnen nur 1/20 ihres Eigengewichts tragen, im Gegensatz zum Menschen,
   der ca. 50% oder mehr des tragen (wenn auch nicht sicher handhaben) kann.
  -Die Preise f�r Industrie Roboter sind, nicht zuletzt aufgrund der billigen PC Hardware,
   um 75% gefallen und werden um weitere 50% fallen auf ca. 10% der urspr�nglichen Kosten.
  -Feedback �ber den Motorstrom erlaubt es, die Masse der Last abzusch�tzen.
  -Wenn das Nachschwingen durch Sensoren verringert w�rde,
   k�nnten die Taktzeiten in den Fabriken reduziert werden.

  -3D Steuerung durch die Maus: links / rechts, oben/unten und vor/zur�ck �ber das Rad!!!

  -Transienten m�ssen sowohl im Fourier- als auch im Zeitraum betrachtet werden.
   Die Wahl des geeigneten Zeitintervalles erfolgt zweckm��ig als
   geometrisches Mittel der typischen Zeiten (z.B. Schwingungsperiode*Abklingzeit)
   Alternativ (aber mit leicht h�herem Aufwand) k�nnen auch die gesamte Transiente
   fourier transformiert werden.

  -Alternative logische Verkn�pfungen: Schnittmenge / AND Verkn�pfung: t-Normen
   ...
   a+b-ab
   -Vereinigungsmenge / OR Verkn�pfung: t-CoNormen bzw. s-Normen
    CoNormen, weil diese Normen durch die Einf�hrung des Komplements
    mit Hilfe der de Morgan Regeln definiert werden.

	Pr�fung von deMorgan:
	!(ab) = !a | !b
	1-(ab) ?= (1-a)+(1-b)-(1-a)(1-b) nein, dazu m��ten a oder b 0 oder 1 sein.
   ...
   abg+(1-1+a+b-ab)(1-g)  "kompensierendes AND"
   abg+(a+b-ab)(1-g)  "kompensierendes AND, wechselt flie�end zwischen AND und OR"

	Nur die Komplementarit�t geht wieder verloren, wie immer!

Zur Defuzzifizierung wird idealerweise der Fl�chenschwerpunkt berechnet.
Das ist aber nur bei einfachen Mengen (Formen) m�glich.
