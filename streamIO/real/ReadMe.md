...
|---|---|
| [AAStreamIn_Float](AAStreamIn_Float.java) | Abstract Random Number Generator using a Float Point (double) Generator and emulating various Generators of<br/>other primitive Types. |
...
| [FilterFloatByFunction](FilterFloatByFunction.java) | Processes the elements of either an input or an output float stream, counting them and optionally mapping them<br/>through an IFloatFunction. |
...
| [FilterFloatExpWindow](FilterFloatExpWindow.java) | An averaging filter with exponential window fall-off, where older values contribute exponentially less to the<br/>running average. |
| [FilterFloatOutliers](FilterFloatOutliers.java) | Continuously filters out statistical outliers beyond a given standard-deviation limit, counting how many were<br/>rejected on each side. |
| [FilterFloatStatistic](FilterFloatStatistic.java) | Bidirectional filter that accumulates running average, variance, skewness and kurtosis from a stream of values<br/>as they pass through it. |
| [FilterFloatSumm](FilterFloatSumm.java) | Filter for Float Numbers. |
| [FilterFloatWindow](FilterFloatWindow.java) | Averages the stream over a fixed-size sliding window in O(1) time per value, using the delay cache to remove<br/>values as they leave the window. |
| [FilterInLin](FilterInLin.java) | Random Number Filter Generator with a white Noise Spectrum, i.e. the Noise Power is constant with the<br/>Frequency A uniform Random Noise Generator also generates a uniform Power Spectrum (heuristic Reason: Spectrum<br/>is as Random as the Input!) Since P(0) = Infinity, the Signal Value could exceed any Bound. |
| [FilterInMul](FilterInMul.java) | Scales an integer random stream by a precomputed factor to produce bounded double values. |
| [FilterIn_Float2Array](FilterIn_Float2Array.java) | Bridges the IStreamIn_Float interface to the IStreamIn interface by converting a stream of float numbers into<br/>a stream of reused fixed-length arrays. |
...
| [FilterIn_FloatByFunction](FilterIn_FloatByFunction.java) | Implements a filter over a float random-number stream that optionally maps each value through an<br/>IFloatFunction before delivery. |
...
| [StreamIn_Float](StreamIn_Float.java) | Combines IStreamIn_Float with markability, ordering and JDK-style IStreamIn semantics into one resettable<br/>float-stream contract. |
...
flowchart TD
    ...
    subgraph detector [detector/]
        DetectorRoot[DetectorThreshold hierarchy]
    end

    subgraph random [random/]
        RandomRoot[ARandomFloat hierarchy]
    end

    AAStreamIn_Float --> AStreamIn_Float
    AStreamIn_Float --> ArrayStreamIn_Float
    AStreamIn_Float --> ConstStreamIn_Float
    AStreamIn_Float --> StreamIn_Arithmetic
    AStreamIn_Float --> StreamIn_Geometric
    ...
    AStreamIn_Float --> FilterIn_FloatByFunction
    AStreamIn_Float --> FilterInMul
    FilterIn_FloatByFunction --> FilterFloatByFunction
    FilterFloatByFunction --> FilterFloatCache
    FilterFloatCache --> FilterFloatDiff
    ...
    FilterFloatCache --> FilterFloatSumm
    FilterFloatByFunction --> FilterFloatDelay
    FilterFloatDelay --> FilterFloatWindow
    FilterFloatDelay --> FilterFloatAverage
    FilterFloatByFunction --> FilterFloatExpWindow
    ...
    FilterFloatByFunction --> FilterFloatStatistic
    FilterOutFloat --> FilterVectorStatistic
    FilterOutFloat --> StreamOutPlotter
    AStreamIn_Float -->|wrapped by| FilterIn_Float2Array
    AStreamIn_Float -->|wrapped by| FilterIn_Float2Object
    ...
    FilterOutFloat -.->|base of| DetectorRoot
    AStreamIn_Float -.->|base of| RandomRoot
