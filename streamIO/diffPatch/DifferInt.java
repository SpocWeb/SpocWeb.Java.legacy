...
public class DifferInt {
	...
	private static final Log L = new Log(DifferInt.class);
	 ...
	 * @return a DiffSetInt describing all the Changes
	 */
	final static public DiffSetInt DIFF(final int[] diff, final int[] curr) {
		return new DifferInt(curr).getDiffs(diff); } 
	...
	final static public DiffSetInt DIFF(final int[] diff, final int[] curr, 
		...
		return new DifferInt(curr).getDiffs(diff, parent); } 
	...
	final static public DiffSetInt DIFF(final int[] diff, final int[] curr, 
		...
		return new DifferInt(curr).getDiffs(diff, parent, branch); } 
	...
	final static public DiffSetInt DIFF(final int[] diff, final int[] curr, 
		...
		return new DifferInt(curr).getDiffs(diff, parent, branch, samePosX); } 
	...
	final static public DiffSetInt DIFF(final int[] diff, final int[] curr, 
		...
		return new DifferInt(curr).getDiffs(diff, parent, branch, samePosX, samePosY); } 
	 ...
	 * @see #compare(int[]) initializes this int. 
	 */
	 ...
	 * @param y the List of Ints to compare
	 * @param startStop
	 * @return the Length of the LCS
	 */
	final public int compare(final int[] y) {
		reSet(); 
		initTracking(); 
		int ret = 0; 
	 ...
	 */
	public int addVal(final int yi) {
	 ...
	 */
	final public DiffSetInt getDiffs(final int[] values) {
	 ...
	 */
	final public DiffSetInt getDiffs(final int[] values, final DiffSet parent
			) throws VersionException {
	 ...
	 * @throws VersionException when you add a Child in the middle of a Branch
	 */
	final public DiffSetInt getDiffs(final int[] values, final DiffSet parent, final String branch
			) throws VersionException {
	 ...
	 * between the current and the given Stream.   
	 * 
	 * @param values the List of Ints to compare 
	...
	final public DiffSetInt getDiffs(final int[] values, final DiffSet parent, final String branch
		...
		final DiffInt[] diffVals = new DiffInt[(vals.length-numSame)+(values.length-numSame)];
		...
		final DiffSetInt ret = new DiffSetInt(diffVals, same, numDeletions, parent, branch, values); 
		return ret; 
	}
	 ...
	 * @param values the Values to fill the DiffInt Values 
	 * @param n ByRef Integer Indices for filling the 'diffVals' and 'same' Arrays.  
	 * @param i
	 * @param j
	 * @return the Number of Deletions in the Differences, to be able to determine the Result Lengh
	 */
	protected final int getDiffs(final DiffInt[] diffVals, final int[][] same, final int[] values
			, final int[] n, final int i, final int j) {
		if (i <= 0) {
			for(int k= -1; ++k < j; ) //
				diffVals[n[0]++] = new DiffInt(vals[k], ~k); 
		    return j; 
		}
		if (j <= 0) {
			for(int k= -1; ++k < i; ) //
				diffVals[n[0]++] = new DiffInt(values[k], k); 
		    return 0; 
		}
		final int j_1 = j-1; 
		final int i_1 = i-1; 
		if (yUp[j].isSet(i_1)) { //ci[j] == c[i_1][j]) {  //
			...
			diffVals[n[0]++] = new DiffInt(values[i_1], i_1); //Addition 
			return ret;
		}
		if (xUp[j].isSet(i_1)) { //ci[j] == ci[j_1]) { //
			final int ret = getDiffs(diffVals, same, values, n, i, j_1); 
			diffVals[n[0]++] = new DiffInt(vals[j_1], ~j_1); //Deletion
			return ret+1;
		}
	...
	final static public void testIt2() {
		final int[] x  = {'A', 'B', 'C', 'D', 'E'}; 
		final int[] x1 = {'A', 'B', 'X', 'D', 'E'};
		final int[] x2 = {'A', 'B', 'C', 'Y', 'E'}; //Mergeable with x1 
		final int[] x3 = {'A', 'B', 'C', 'Z', 'E'}; //Conflict with x2
		final DifferInt  xDiffer = new DifferInt(x); 
		final DiffSetInt x1_x = xDiffer.getDiffs(x1); 
		//No Conflict
		//final DifferInt  xDiffer2 = new DifferInt(x); 
		final DiffSetInt x2_x = xDiffer.getDiffs(x2); 
		final DiffSet Conflicts1 = x1_x.applyDiffs(x2_x); //x1_x is modified internally here! 
		Assert.EQUALS(0, Conflicts1.getInt()); 
		final int[] x4 = x1_x.merge(x2); 
		Assert.EQUALS(new int[] {'A', 'B', 'X', 'Y', 'E'} , x4);
		//Conflict
		//final DifferInt  xDiffer3 = new DifferInt(x); 
		final DiffSetInt x3_x = xDiffer.getDiffs(x3); 
		final DiffSetInt Conflicts2 = (DiffSetInt) x3_x.applyDiffs(x2_x); 
		Assert.EQUALS(2, Conflicts2.getInt()); Conflicts2.toString(); 
		final int[] x5 = x3_x.merge(x2); 
		Assert.EQUALS(new int[] {'A', 'B', 'C', 'Z', 'E'} , x5);
		final int[] x6 = Conflicts2.merge(x5); 
		Assert.EQUALS(new int[] {'A', 'B', 'C', 'Y', 'E'} , x6);
	} 
	
	/** tests all Methods of this Class	 */
	final static public void test1() throws VersionException {
		final int[] Y = {'A', 'B', 'C', 'B', 'D', 'A', 'B'}; 
		final int[] X = {'B', 'D', 'C', 'A', 'B', 'A'};
		testIt(Y, X); 
		testIt(X, Y);
		/*
		final DifferInt yDiff = new DifferInt(Y); 
		yDiff.compare(X); 
		final int[][] yDiffs = yDiff.getDiffs();
		final int[] delValX = VectorInt.GET_AT(X, yDiffs[3]); 
		
		final DifferInt xDiff = new DifferInt(X); 
		xDiff.compare(Y); 
	...
	/** tests all Methods of this Class	 */
	private static final void testIt(final int[] x, final int[] y) throws VersionException {
		final DifferInt xDiffer = new DifferInt(x); 
		Assert.EQUALS(x.length, xDiffer.compare(x)); 
		final DiffSetInt diff = xDiffer.getDiffs(y, null, null, true, true);
		Assert.EQUALS(4, diff.same[0].length); 
		Assert.EQUALS(4, diff.same[1].length); 
		final int[] merged = diff.merge(x); 
		Assert.EQUALS(y, merged); 
		//VectorInt.CPL_AT(sameDiffVals[0]); 
		final int[] mergedBack = diff.merge(y, true); //mergeDiff(y, sameDiffVals[0], sameDiffVals[1], sameDiffVals[0].length-numDel); 
