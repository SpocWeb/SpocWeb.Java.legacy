...
public class ScalarPlot {
	...
	public ScalarPlot(AGraph2D g_) { this(g_, null); }
	 ...
	 */
	public ScalarPlot(AGraph2D g_, Color[] Palette_) {
	 ...
	 */
	public void ScalarRow (int x0, int x1, int z0, int z1) {
		boolean zStep, color = false;
		int dx, dz, d;
		setPaletteColor(z0);
		if (z1 == z0) {
			g.P.x = x0; g.drawHLine(x1); return; }	//faster Routines, don't use Clipping
		...
		d = dx-dz; g.P.x = x0;	//Set the Start to the Start Point so that at least the last Draw Action
		while (x0 < x1) //no longer guaranteed to come along x0 AND z0 at the same time!
		{	//no clipping anymore!
			if (d <  0) {int n1 = 1- d/dx; z0+= zStep ? n1 : -n1; d += n1*dx; color = true;}
			if (d >= 0) {x0++; d -= dz;
				if (color) {	//Wait and accumulate Changes in Color and x-Coordinate
					...
					if (x0 > g.P.x+1) {
						...
						g.P.x = x0; g.setPixel(); }	//Optimization: for a single Pixel don't call drawLine!
					color = false;
				}
			}
		}
		...
		while (y0 < y1) { //no longer guaranteed to come along y0 AND z0 at the same time!
			...
			&& (d1 >= 0)){
				...
				if ((z0Old == z00) &&
					...
					ScalarRow (x0, x1, z00, z10, 0, 0); }
				z0Old =  z00;
				z1Old =  z10; }
		}
	}

	/**Paints the Scalar Values.
	 ...
	 * This is derived from the drawTriangle Routine from 'Graph2D.AGraph2D' . */
	public void ScalarLine (int x0, int x1, int y0, int y1, int z0, int z1) {
		...
		g.P.y = y0-1; //include this one Pixel
		while(++g.P.y < y1)		//Now paint from the upper Point to the Middle and from there to the End
		{	//no clipping anymore! yStep == 1	adding below because of the autoIncrement ++g.P.y
			...
			ScalarRow(Px1, Px2, Pz1, Pz2, 0, 0); Px1 = Px2; Pz1 = Pz2;	//draw a horizontal Line!
		}
		g.P = End;	//Set the current Point to the End Point
	}
	...
	public void ScalarTriangle (int x0, int x1, int x2,
		...
		g.P.y = y0; //Set one Pixel
		while(++g.P.y < y2)		//Now paint from the upper Point to the Middle and from there to the End
		{	//no clipping anymore! yStep == 1
			d1 -= dx1; dd1-= dz1;	//added because of the autoIncrement ++g.P.y
			d2 -= dx2; dd2-= dz2;
			if (d2 <  0) {int n2 = 1- d2/dy2; Px2+= xStep2 ? n2 : -n2; d2 += n2*dy2;}
			if (dd2<  0) {int n2 = 1-dd2/dy2; Pz2+= zStep2 ? n2 : -n2; dd2+= n2*dy2;} if ((g.P.y != y1)	&& (dy1 != 0)) {
			...
			ScalarRow(Px1, Px2, Pz1, Pz2);	//draw a horizontal Line!
		}
	}
	...
	/**Paints the Picture of Integer Values
	 *
	 * The Picture is painted columnwise.
	 */
	public void paintPicture(Point2D Offset, int[][]zP) {
		int[] column;
		int newColor; //, oldColor =  g.getColor().getRGB();
		int x = -1; g.P.x = Offset.x-1;
		while (++x < zP.length) {
			int y = -1; g.P.y = Offset.y-1;
			++g.P.x; column = zP[x];
			while (++y < zP[0].length) {
				++g.P.y; newColor = column[y];
...
//						Picture[SF.x][SF.y] =
	 ...
	 * Also the Changes in the x-Coordinate are collected until the Color changes.
	 */
	public void ScalarRow	(int x0, int x1, int z0, int z1, int u0, int u1) {
		boolean color = false;
		int dx, dz, du, zStep, uStep, zd, ud, d;
		setPaletteColor(z0);
		if (z1 == z0) { g.P.x = x0; g.drawHLine(x1); return; }	//faster Routines, don't use Clipping
		if (x1 <  x0) { //Linien werden unabh. von der Reihenfolge der Punkte gezeichnet
			//man kann x0 stets inkrementieren,statt etwas zu addieren
			d = x1; x1 = x0; x0 = d;
			d = z1; z1 = z0; z0 = d;
		...
		zd = dx-dz; ud = dx-du; g.P.x = x0;	//Set the Start to the Start Point so that at least the last Draw Action
		while (x0 < x1) { //no longer guaranteed to come along x0 AND z0 at the same time!
			//no clipping anymore!
...
//		ScalarTriangle (x0, x1, x2, y0, y1, y2, z0, z1, z2, 0, 0, 0);}
	...
	public void ScalarTriangle (int x0, int x1, int x2,
		...
		while(++g.P.y < y2) {	//Now paint from the upper Point to the Middle and from there to the End
			...
			if (zd2 <  0) {int n2 = 1- zd2/dy2; Pz2+= zStep2 ? n2 : -n2; zd2+= n2*dy2;} if ((g.P.y != y1) && (dy1 != 0)) {
			...
			ScalarRow(Px1, Px2, Pz1, Pz2, Pu1, Pu2);	//draw a horizontal Line!
		}
	}
	...
	public static void testIt(String[] args) throws java.io.IOException {
		System.out.println("Testing " + ScalarPlot.class.getName());
	}
	...
	public static void paintScalarObjects(AGraph2D g, Color[] Palette) {
		ScalarPlot SP = new ScalarPlot(g, Palette);
		g.P.y = 200;
		SP.ScalarRow     (  0, 600,   0, 299);	//continuous Coloring of a horizontal Line
