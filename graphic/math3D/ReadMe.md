...
| [Body3DPainter](Body3DPainter.java) | Title: Body3DPainter Description: Painter Object, receives or generates a Viewer Window to output the Body3D<br/>Model referenced. |
...
| [ISpatial](ISpatial.java) | Interface that defines Methods of Figures that need to be rastered and drawn into a GUI or placed into or<br/>evaluated at a Position. |
...
| [TestOdePlotter](TestOdePlotter.java) | Title: TestOdePlotter Description: Purpose: Purpose / Responsibilities of this Class Design Decisions /<br/>Implementation Details: If similar Classes exist (e.g. Polymorphism), characterize the specific Differences to<br/>compare these. |
...
| [Wire3D](Wire3D.java) | Title: Wire3D Description: Adds a Description of the Wireframe Lines to the Points in Polygon3D This is<br/>identical to the Model in @see graphic.math2D.Frame2DMap Design Decisions / Implementation Details: If similar<br/>Classes exist (e.g. Polymorphism), characterize the specific Differences to compare these. |
| [testMathGraph2](testMathGraph2.java) | Title: testMathGraph2 Description: Purpose: Purpose / Responsibilities of this Class Design Decisions /<br/>Implementation Details: If similar Classes exist (e.g. Polymorphism), characterize the specific Differences to<br/>compare these. |
| [testMathGraph3D](testMathGraph3D.java) | Title: testMathGraph3D.java Description: Older Class for testing MathGraph3 replaced by TestMathGraph3 Known<br/>SubClasses: Copyright: Copyright (c) Matthias Heuer Company: personal Created on 06-28-1999, 12:52 PM |
...
flowchart TD
  subgraph math3D
    ICoordMapper["ICoordMapper (interface)"]
    Coordinates2D_["Coordinates2D_"]
    ...
    Body3D["Body3D"]
    Platonic["Platonic"]
    ISpatial["ISpatial (interface)"]
    ASpatial["ASpatial"]
    Figures3D["Figures3D"]
    Column3D["Column3D"]
    PlanePlotter["PlanePlotter"]
    ScalarPlanePlotter["ScalarPlanePlotter"]
    PlotVector["PlotVector"]
    HistoPlot["HistoPlot"]
    ScalarPointPlot["ScalarPointPlot"]
    VoxelPlot["VoxelPlot"]
    OdePlotter["OdePlotter"]
    HyperPlanePlotter["HyperPlanePlotter"]
    Body3DPainter["Body3DPainter"]
    FloatPointKeyController["FloatPointKeyController"]
    RotationMouseController["RotationMouseController"]

    Coordinates2D_ -->|"implements"| ICoordMapper
    Coordinates3D -->|"implements, uses"| ICoordMapper
    linkStyle 0 opacity:1
    Coordinates3D -->|"delegates rotation/projection to"| Projection
    Projection -->|"extends"| Line
    Polygon3D -->|"caches Extent as"| Line
    Body3D -->|"extends"| Polygon3D
    Body3D -.->|"built from coordinate tables in"| Platonic
    ASpatial -->|"implements"| ISpatial
    linkStyle 5 opacity:1
    Column3D -->|"maps via"| ICoordMapper
    Figures3D -->|"maps via"| ICoordMapper
    PlanePlotter -->|"extends"| ASpatial
    ScalarPlanePlotter -->|"extends"| PlanePlotter
    PlotVector -->|"extends"| ASpatial
    linkStyle 10 opacity:1
    HistoPlot -->|"extends, draws via"| Column3D
    ScalarPointPlot -->|"extends"| ASpatial
    VoxelPlot -->|"extends"| ASpatial
    OdePlotter -->|"extends"| ASpatial
    HyperPlanePlotter -->|"extends"| ASpatial
    linkStyle 15 opacity:1
    Body3DPainter -->|"maps Body3D via"| Coordinates3D
    Body3DPainter -->|"wires up"| FloatPointKeyController
    Body3DPainter -->|"wires up"| RotationMouseController
  end
```
