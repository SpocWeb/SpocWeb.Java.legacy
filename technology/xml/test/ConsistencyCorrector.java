 ...
 * und updated ggf. die �ltere. 
 * 
 * Hier wird gepr�ft, ob beide Werte zusammenpassen, sofern beide ver�ndert wurden. 
 * Passen sie nicht und wurde nur einer ver�ndert, wird der jeweils andere nachgezogen, 
 * wurde keiner ver�ndert, wird gar nichts gemacht (au�er man hat den Debug Modus an). 
 * Wird die passende EKP-Nummer oder Rise-ID nicht gefunden 
 * oder passen die Werte nicht zusammen, wird �ber den Return String USER_WARNING zur�ckgeliefert.
 * 
 * Die Klasse stellt Dummy Methoden mit auskommentierten Datenbank- Zugriffen bereit, 
 * um die bijektive Abbildung der IDs aufeinander auszuf�hren. 
 * 
 * <!-- docstate
 * pass: 2
...
*/	}
   ...
   /**
	* Extra Pr�fung f�r von redundante Beziehungen id1 und id2: 
	* * Beteiligung (Konzern), 
	* * Master (Dublette), 
	* * Verband, 
	* * Zentrale (f�r Filialen) 
	...
	* Hier wird gepr�ft, ob beide Werte zusammenpassen, sofern beide ver�ndert wurden. 
	* Wurde nur einer ver�ndert, wird der andere nachgezogen, 
	* wurde keiner ver�ndert, wird gar nichts gemacht (au�er man hat den Debug Modus an). 
	* Wird die passende EKP-Nummer oder Rise-ID nicht gefunden 
	* oder passen die Werte nicht zusammen, wird �ber den Return String ReturnXML.WARNING zur�ckgeliefert.   
	...
	*/
	...
	/**
	 * Pr�ft die Konsistenz der Beteiligung des Unternehmens
	 * @param unternehmung
	...
	final static public String areIdsConsistent(final Kunde unternehmung, final IInvertAble mapperRise2Ekp) {
		...
		if (null != result) { //not correctable, formulate the Error Message
			if (USER_WARNING.equals(result)){
				...
				return "Inkonsistente Beteiligungen: EKP-Nr des Konzerns, ermittelt �ber die ZkdbID:"+result+
						"\ndirekte Angabe des Konzerns �ber EKP-Nr:"+unternehmung.getAnotherID();
			}
		} 
		//correctable...
