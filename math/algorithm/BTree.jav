...
/**Implementation of a balanced Tree
...
Btree::~Btree(void){if( root != 0 ) delete root;}
...
void BtreeIterator::restart() { index = 0; }

Object& BtreeIterator::operator++() { return beingIterated[++index]; }

Object& BtreeIterator::operator++( int ) { return beingIterated[index++]; }

Object& BtreeIterator::current() { return beingIterated[index]; }
...
class Node {
    ...
    Node( int b, InnerNode P, Btree T = null );

    void add(Sortable , int );
    void remove(int );

    Object & operator[]( long i ) ;
    Object & found( Sortable ,
                                 Node ,
                                 int
                               );

    long findRank( Sortable ) ;
    long nofKeys() ; // # keys in or below this node

    LeafNode firstLeafNode() = null;
    LeafNode lastLeafNode() = null;
    ...
    int last;   // for inner node 1 <= last <= InnerMaxIndex
                ...
                //  updated)
    InnerNode *parent; // a parent is always an inner node (or 0 for the root)
    Btree *tree;   // the tree of which this node is a part
...
public:
    Item();
    Item(Node n, Sortable o);
    Item(Sortable o, Node n);
    // data
...
public:
    InnerNode( InnerNode *, Btree * = 0 );
    InnerNode( InnerNode *, Btree *, Node * );
    ~InnerNode();

    void add( Sortable *, int );
    void add( Item &, int );
    void add( int, Sortable *, Node * );
    void addElt( Item &, int );
    void addElt( int, Sortable *, Node * );
    void remove( int );
    void removeItem( int );

    Object & operator[]( long i ) ;
    Object & found( Sortable *, Node * *, int * );

    long nofKeys( int i ) ;
    void setTree( int i, Node * node );
    void setKey( int i, Sortable * obj );
    void setItem( int i, Item & itm );
    void setItem( int i, Sortable * obj, Node * node );
    long getNofKeys( int i ) ;
    void setNofKeys( int i, long r );
    long incNofKeys( int i, long N=1 );
    long decNofKeys( int i, long N=1 );
    long findRank( Sortable * ) ;
    long findRank_bu(  Node * ) ;
    Node *getTree( int i ) ;
    Sortable *getKey( int i ) ;
    Item & getItem( int i ) ;


    int  indexOf(  Node * ) ;
    void incrNofKeys( Node * np );
    void decrNofKeys( Node * np );
    long nofKeys() ;
    ...
    void split();
    void splitWith( InnerNode *, int );
    void mergeWithRight( InnerNode *, int );
    void balanceWithLeft( InnerNode *, int );
    void balanceWithRight( InnerNode *, int );
    void balanceWith( InnerNode *, int );
    void pushLeft( int cnt, InnerNode * leftsib, int parentIdx );
    void pushRight( int cnt, InnerNode * rightsib, int parentIdx );
    void appendFrom( InnerNode *, int, int );
    void append( Sortable *, Node * );
    void append( Item & );
    void shiftLeft( int );
	...
	private Item item; // actually items[maxIndex()+1] is desired
};

class LeafNode extends Node {
public:

    LeafNode(InnerNode * P, Sortable * obj = 0, Btree * T = 0 );
    ~LeafNode();
    ...
    long nofKeys() ;
    long findRank( Sortable * ) ;
    Sortable *getKey( int idx ) { return item[idx]; }
    void setKey( int idx, Sortable * obj ) { item[idx] = obj; }

    int indexOf(  Sortable * ) ;

    LeafNode *firstLeafNode();
    LeafNode *lastLeafNode();

    void split();
    void splitWith( LeafNode *, int );
    void mergeWithRight( LeafNode *, int );
    void balanceWithLeft( LeafNode *, int );
    void balanceWithRight( LeafNode *, int );
    void balanceWith( LeafNode *, int );
    void pushLeft( int cnt, LeafNode *, int parentIndex );
    void pushRight( int cnt, LeafNode *, int parentIndex );
    void appendFrom( LeafNode *, int, int );
    void append( Sortable * );
    void shiftLeft ( int );
    ...
    Sortable * *item; // actually Sortable* item[maxIndex()+1] is desired
};

class Btree extends Collection {
public:
    Btree( int ordern = 3 );//-create a Btree of order n
    ~Btree();

    void add( Object & );
    void detach( Object &, DeleteType = NoDelete );
    void flush( DeleteType = DefDelete );
    int hasMember( Object & ) ;
    Object & findMember( Object & ) ;

    int isEmpty()  { return itemsInContainer == 0; }
    countType getItemsInContainer()  { return itemsInContainer; }

    classType isA()  { return btreeClass; }
    char *nameOf()  { return "Btree"; }
    int isEqual(  Object & ) ;
    void printOn( ostream & ) ;
    ...
    protected void incrNofKeys() { itemsInContainer++; }
    protected void decrNofKeys() { itemsInContainer--; }

    protected long i_add(  Object & );
    ...
    private int Order;          //-the order of the tree (should be > 2)
    private int Order2;         //-always == order*2+1 (assumes a memory access
                        // is cheaper than a multiply and increment by one
    private int Inner_LowWaterMark;
    private int Leaf_LowWaterMark;
    private int Inner_MaxIndex;
    private int Leaf_MaxIndex;
 ...
 Node InnerNode::getTree( int i )  {return item[i].tree;}

 Sortable InnerNode::getKey(int i) {return item[i].key;}

 Item InnerNode::getItem(int i) {return item[i];}

 void InnerNode::setTree(int i, Node node){item[i].tree = node; node->parent = this;}

 void InnerNode::setKey(int i, Sortable obj) {item[i].key = obj;}

 void InnerNode::setItem(int i, Item itm) {item[i] = itm; itm.tree->parent = this;}

 void InnerNode::setItem(int i, Sortable obj, Node node) {setTree(i, node); setKey(i, obj);}

 long InnerNode::getNofKeys(int i) {return item[i].nofKeysInTree;}

 void InnerNode::setNofKeys(int i, long r) {item[i].nofKeysInTree = r;}

 long InnerNode::incNofKeys(int i, long N) {return ( item[i].nofKeysInTree += N );}

 long InnerNode::decNofKeys(int i, long N) {return ( item[i].nofKeysInTree -= N );}

 long InnerNode::nofKeys(int i) {return getNofKeys(i);}

 int InnerNode::Psize() {return last;}

 int InnerNode::Vsize() {return Psize()+1;}

 int InnerNode::maxIndex() {return tree->Inner_MaxIndex;}

 int InnerNode::maxPsize() {return tree->Inner_MaxIndex;}

 int InnerNode::isFull() {return last == maxIndex();}

 int InnerNode::isAlmostFull() {return last >= maxIndex() - 1;}

 int InnerNode::isLow() {return last < tree->Inner_LowWaterMark;}

 void LeafNode::removeItem( int i){remove(i);}

 Object & LeafNode::operator[]( long i ) {return *((Object *)item[(int)i]);}   // CHECK - cast to int OK?

 int LeafNode::Psize() {return last+1;}

 int LeafNode::Vsize() {return Psize()+1;}

 int LeafNode::maxIndex() {return tree->Leaf_MaxIndex;}

 int LeafNode::maxPsize() {return tree->Leaf_MaxIndex + 1;}

 int LeafNode::isFull() {return last == maxIndex();}

 int LeafNode::isAlmostFull() {return last >= maxIndex() - 1;}

 int LeafNode::isLow() {return last < tree->Leaf_LowWaterMark;}

 int Btree::order() {return Order;}

class BtreeIterator
extends ContainerIterator {

    public BtreeIterator(  Btree );
	...
	private Btree beingIterated;
    long index;
};

BtreeIterator::BtreeIterator(  Btree & toIterate ) :
beingIterated( toIterate ), index( 0 ) { }

Object Btree::operator[]( long i ) const { return (*root)[i]; }
