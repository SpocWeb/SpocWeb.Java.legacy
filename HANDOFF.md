...
| knowledge/CachedValue.java | CachedValue | assertIsDirty(boolean) | 69 | The null test is inverted - `calculator.run()` is only reached when `calculator` is null, so the method throws NullPointerException whenever no calculator is set, and never invokes one that is set; the branches belong the other way round. | High |
| knowledge/BasicAttribute.java | BasicAttribute | getType() | 102 | Keyed by `StatusID` instead of `TypeID`, and routed through `FactoryStatus`, so it resolves and caches the Status row as this attribute's Type whenever the two IDs differ - the normal case. | High |
| knowledge/MetricAttribute.java | MetricAttribute |  | 43 | Static `Fields` is seeded with an empty array instead of `BasicAttribute.Keys`, so `Fields()` returns only the class's own Value column despite its "including Parent Fields" contract; the inherited TypeID/SubjectID/StatusID key columns are missing from every generic statement `DBObjectFactory` builds from it. `MetaType`, which concatenates `Status.Fields`, shows the intended form. | High |
| knowledge/StringAttribute.java | StringAttribute |  | 43 | Static `Fields` is seeded with an empty array instead of `BasicAttribute.Keys`, so `Fields()` returns only the class's own Value column despite its "including Parent Fields" contract; the inherited TypeID/SubjectID/StatusID key columns are missing from every generic statement `DBObjectFactory` builds from it. `MetaType`, which concatenates `Status.Fields`, shows the intended form. | High |
| knowledge/TimeAttribute.java | TimeAttribute |  | 44 | Static `Fields` is seeded with an empty array instead of `BasicAttribute.Keys`, so `Fields()` returns only the class's own Value column despite its "including Parent Fields" contract; the inherited TypeID/SubjectID/StatusID key columns are missing from every generic statement `DBObjectFactory` builds from it. `MetaType`, which concatenates `Status.Fields`, shows the intended form. | High |
| knowledge/EnumAttribute.java | EnumAttribute |  | 48 | Static `Fields` is seeded with an empty array instead of `BasicAttribute.Keys`, so `Fields()` returns only the class's own Value column despite its "including Parent Fields" contract; the inherited TypeID/SubjectID/StatusID key columns are missing from every generic statement `DBObjectFactory` builds from it. `MetaType`, which concatenates `Status.Fields`, shows the intended form. | High |
| knowledge/DBObjectFactory.java | DBObjectFactory | Condition(IPrimaryKey, String) | 239 | Key values are concatenated into the SQL text with no quoting and no escaping, so any String or Date key containing a quote breaks the statement and any attacker-controlled key value injects SQL; `insertObject` and `updateObject` do the same with data values. | High |
| knowledge/DBObjectFactory.java | DBObjectFactory | insertObject(PersistAble) | 383 | The column list is built keys-then-fields ascending, but the value list appends fields first and then keys, each descending, so values line up with the wrong columns for any table with more than one column. | High |
| knowledge/Objekt.java | Objekt | getType() | 131 | Keyed by `StatusID` instead of `TypeID` and routed through `FactoryStatus` rather than `FactoryType`, so it resolves and caches the Status row as this object's Type whenever the two IDs differ. Identical to the `BasicAttribute.getType` defect. | High |
| knowledge/Status.java | Status | finalize() | 132 | Empty in both branches, so nothing is ever written back on finalization, while `DbCachedFactory` caches weakly on the stated assumption that a dirty object persists itself before collection; changes to a collected object are lost silently. | Medium |
| knowledge/DbCachedFactory.java | DbCachedFactory | DbCachedFactory(Connection, PersistAble) | 115 | The `SELECT Max(ID)` failure is swallowed whole, leaving `MaxID` at 0; a caller cannot tell a table whose largest ID really is 0 from one whose query failed, and the documented 0-means-insert convention then applies to every object. | Medium |
| knowledge/IdKey.java | IdKey | newInstance(IPrimaryKey) | 147 | The block comment disabling the preceding dead code is never closed, so it runs on through the following Javadoc and swallows it; `newInstance` is left undocumented in the generated API and `list-todo` reports it as missing a comment. | Low |
